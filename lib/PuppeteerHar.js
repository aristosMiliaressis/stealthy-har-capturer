...
class PuppeteerHar {
    ...
    async start({ path, saveResponse } = {}) {
        ...
        network_observe.forEach(method => {
            this.client.on(method, params => {
                if (!this.inProgress) {
                    console.log(`inProgress ${method}`)
                    return;
                }
                this.network_events.push({ method, params });

                if (this.saveResponse && method == 'Network.dataReceived') {
                    const promise = this.client
                        .send('Network.getResponseBody', { requestId: params.requestId })
                        .then((responseBody) => {
                            var responseReceivedEvent = this.network_events.filter((evt) => evt.method == "Network.responseReceived" && evt.params.requestId == params.requestId)[0]

                            // Response body is unavailable for redirects, no-content, image, audio and video responses
                            if (responseReceivedEvent && responseReceivedEvent.params && responseReceivedEvent.params.response.status !== 204 && responseReceivedEvent.params.response.headers.location == null) {
                                // Set the response so `chrome-har` can add it to the HAR
                                responseReceivedEvent.params.response.body = new Buffer.from(
                                    responseBody.body,
                                    responseBody.base64Encoded ? 'base64' : undefined).toString();
                            }
                        }, (reason) => {
    ...
    async stop() {
        ...
        har.log.entries.forEach(entry => {
            entry.request["_resourceType"] = entry.request["_initiator_type"];
        });
        this.cleanUp();
        if (this.path) {
